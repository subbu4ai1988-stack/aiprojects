...
type Runtime={environment:string;database:string;max_upload_mb:number;ai_calls_per_minute:number;ai_monthly_token_budget:number;candidate_retention_days:number;privacy_auto_delete:boolean};
...
export default function Admin({job,close}:{job?:Job;close:()=>void}){
  ...
  const load=async()=>{
    try{
      setError('');
      const [loadedUsers,loadedRuntime,loadedUsage,loadedPrivacy,loadedCandidates,loadedAudit]=await Promise.all([
        api('/admin/users'),api('/admin/runtime'),api('/admin/ai/usage'),api('/admin/privacy/summary'),api('/admin/privacy/candidates'),api('/admin/privacy/audit?limit=12')
      ]);
      setUsers(loadedUsers);setRuntime(loadedRuntime);setUsage(loadedUsage);setPrivacy(loadedPrivacy);setPrivacyCandidates(loadedCandidates);setAudit(loadedAudit);
